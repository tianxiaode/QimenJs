import{A as v,a as g}from"./abp-BjLiPpgK.js";import{S as m,a as u}from"./spring-ChL9TfTS.js";import{L as p,a as h}from"./local-DemgwnIB.js";import{D as y}from"./tree-CYmgy4ES.js";import{r as f,o as d}from"./index-OvYsTaGM.js";import"@/composable";import"@/error";import"@qimenjs/error";import"@qimenjs/registry";import"@/registry/registrars/DomainRegistrar";const n=new v,c=new g,l=new m,r=new u,o=new y,s=new p,i=new h;function S(){w(),f(`
        ...
        <div class="stat-grid">
            ...
            <div class="stat-card">
                ...
                <div class="stat-value" id="stat-notifications">${s.items.length}</div>
                <div class="stat-change up">LocalReadonlyEntityManager</div>
            </div>
            <div class="stat-card">
                <div class="stat-label">本地标签</div>
                <div class="stat-value" id="stat-tags">${i.items.length}</div>
                <div class="stat-change up">LocalCrudEntityManager</div>
            </div>
            <div class="stat-card">
                <div class="stat-label">认证状态</div>
                <div class="stat-value" style="font-size:16px;">${d.isAuthenticated()?"✓ 已认证":"✗ 未认证"}</div>
                <div class="stat-change ${d.isAuthenticated()?"up":"down"}">OAuth2 ${d.isAuthenticated()?"Token 有效":"未登录"}</div>
            </div>
        </div>
    ...
    `),b()}function w(){const a=[{id:1,title:"系统维护通知",message:"系统将于今晚 22:00 进行维护",type:"system",read:!1,createdAt:"2026-07-03 09:00"},{id:2,title:"新功能上线",message:"树形管理器已上线，支持懒加载",type:"feature",read:!1,createdAt:"2026-07-02 14:30"},{id:3,title:"安全提醒",message:"请定期修改密码",type:"security",read:!0,createdAt:"2026-07-01 10:00"},{id:4,title:"版本更新",message:"v2.0 已发布，新增 5 种 Manager 类型",type:"feature",read:!0,createdAt:"2026-06-30 16:00"}];s.sourceData.clear(),a.forEach(e=>s.sourceData.set(e.id,e)),s.items=[...a];const t=[{id:1,name:"重要",color:"#f44336",count:5},{id:2,name:"待办",color:"#FF9800",count:12},{id:3,name:"已完成",color:"#4CAF50",count:28},{id:4,name:"进行中",color:"#2196F3",count:8}];i.sourceData.clear(),t.forEach(e=>i.sourceData.set(e.id,e)),i.items=[...t]}async function b(){try{const a=await n.list(),t=document.getElementById("stat-users");t&&(t.textContent=String(n.total??a.length))}catch{}try{const a=await c.list(),t=document.getElementById("stat-products");t&&(t.textContent=String(c.total??a.length))}catch{}try{const a=await l.list(),t=document.getElementById("stat-orders");t&&(t.textContent=String(l.total??a.length))}catch{}try{const a=await r.list(),t=document.getElementById("stat-items");t&&(t.textContent=String(r.total??a.length))}catch{}try{const a=await o.list();o.ingest(a);const t=document.getElementById("stat-departments");t&&(t.textContent=String(a.length))}catch{}}export{S as renderDashboard};
