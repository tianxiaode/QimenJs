import{r as d}from"./index-OvYsTaGM.js";import"@qimenjs/error";import"@qimenjs/registry";import"@/registry/registrars/DomainRegistrar";function l(){d(`
        <div class="page-header">
            <h2>数据处理器</h2>
            <p>@qimenjs/data-processor + data-processor-abp + data-processor-spring</p>
        </div>
  ...
  "last": false
    ...
    `)}window.__processAbp=()=>{const s=document.getElementById("dp-abp-result");if(s)try{const e=document.getElementById("dp-abp-input").value,t=JSON.parse(e),a={items:t.items||[],total:t.totalCount||0,page:1,pageSize:(t.items||[]).length};s.innerHTML=`
            <div><span class="badge badge-success">处理完成</span></div>
            <pre style="background:#0A0A0B;padding:12px;border-radius:6px;overflow:auto;font-size:12px;color:#A855F7;">${JSON.stringify(a,null,2)}</pre>
        `}catch(e){s.innerHTML=`<span class="badge badge-danger">处理失败: ${e}</span>`}};window.__processSpring=()=>{const s=document.getElementById("dp-spring-result");if(s)try{const e=document.getElementById("dp-spring-input").value,t=JSON.parse(e),a={items:t.content||[],total:t.totalElements||0,page:(t.number||0)+1,pageSize:t.size||10,pages:t.totalPages||0};s.innerHTML=`
            <div><span class="badge badge-success">处理完成</span></div>
            <pre style="background:#0A0A0B;padding:12px;border-radius:6px;overflow:auto;font-size:12px;color:#A855F7;">${JSON.stringify(a,null,2)}</pre>
        `}catch(e){s.innerHTML=`<span class="badge badge-danger">处理失败: ${e}</span>`}};export{l as renderDataProcessor};
