...
exports.copyIfDefined = copyIfDefined;
exports.replaceMembers = replaceMembers;
exports.getNestedValue = getNestedValue;
exports.setNestedValue = setNestedValue;
exports.destroyMembers = destroyMembers;
...
function copyIfDefined(source, target) {
    for (const key in source) {
        if (source.hasOwnProperty(key)) {
            if ((0, base_1.isObject)(source[key])) {
 ...
 */
function replaceMembers(source, target, maxDepth = 10, currentDepth = 0) {
    for (const key in source) {
        if (source.hasOwnProperty(key)) {
 ...
 */
function destroyMembers(obj, members) {
    members.forEach((member) => {
        if (obj.hasOwnProperty(member)) {
            let memberValue = obj[member];
            // 如果成员有 destroy 方法，则调用它
            if ((0, base_1.isObject)(memberValue) &&
