...
function formatNumber(value, format) {
    ...
    const sign = num < 0 ? "-" : "";
    const result = formattedDec.length > 0
 ...
 * @returns 格式化后的整数部分
 */
function formatInteger(digits, format, useComma) {
 ...
 * @returns 格式化后的小数部分
 */
function formatDecimal(digits, format) {
 ...
 * @returns 格式化后的货币字符串
 */
function formatCurrency(number, currency, format) {
