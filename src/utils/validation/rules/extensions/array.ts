...
export interface UniqueByRuleOptions<T = any> extends Omit<ArrayRuleOptions, 'required' | 'nullable'> {
    /**
     * 指定用于唯一性比较的属性名或计算函数
     * 
 ...
 * ```
 */
export interface SortedRuleOptions<T = any> extends Omit<ArrayRuleOptions, 'required' | 'nullable'> {
    /**
     * 排序方式或自定义排序函数
     * 
     * - 'asc': 升序排列
