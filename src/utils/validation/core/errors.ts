...
import { ValidationErrorContext, ValidationRuleError } from './types'
...
export const ValidationErrorBuilder = {
   ...
   * 创建类型不匹配错误
   */
  type_mismatch(expectedType: string, actualType: string, context?: ValidationErrorContext): ValidationRuleError {
   ...
   * 创建值过小错误
   */
  too_small(min: number, value: any, exclusive: boolean = false, context?: ValidationErrorContext): ValidationRuleError {
   ...
   * 创建值过大错误
   */
  too_large(max: number, value: any, exclusive: boolean = false, context?: ValidationErrorContext): ValidationRuleError {
   ...
   * 创建超出范围错误
   */
  out_of_range(min: number, max: number, value: any, context?: ValidationErrorContext): ValidationRuleError {
   ...
   * 创建格式无效错误
   */
  invalid_format(field: string, value: any, format: string, context?: ValidationErrorContext): ValidationRuleError {
   ...
   * 创建模式不匹配错误
   */
  pattern_mismatch(pattern: string | RegExp, value: any, context?: ValidationErrorContext): ValidationRuleError {
   ...
   * 创建不允许值错误
   */
  not_allowed(value: any, allowedValues: any[], context?: ValidationErrorContext): ValidationRuleError {
   ...
   * 创建条件失败错误
   */
  condition_failed(field: string, condition: string, value: any, context?: ValidationErrorContext): ValidationRuleError {
   ...
   * 创建自定义错误
   */
  custom(customCode: string, message: string, context?: ValidationErrorContext): ValidationRuleError {
   ...
   * 创建密码太短错误
   */
  password_too_short(minLength: number, actualLength: number, context?: ValidationErrorContext): ValidationRuleError {
