...
function calculateSHA512(str) {
    ...
    class Uint64 {
        ...
        static rotr(x, n) {
            n %= 64; // 确保旋转位数在0-63之间
            if (n === 0)
                return new Uint64(x.hi, x.lo);
            if (n < 32) {
                return new Uint64((x.hi >>> n) | (x.lo << (32 - n)), (x.lo >>> n) | (x.hi << (32 - n)));
            }
            else {
                // n >= 32: 交换高低32位并旋转剩余位数
                const remaining = n - 32;
                return new Uint64((x.lo >>> remaining) | (x.hi << (32 - remaining)), (x.hi >>> remaining) | (x.lo << (32 - remaining)));
            }
        }
        // 右移
        static shr(x, n) {
    ...
    // 将32位数组转换为64位数组
    function uint32ArrayToUint64Array(arr) {
        const result = [];
    ...
    for (let i = 0; i < msg64.length; i += 16) {
        ...
        // 扩展到80个字
        for (let j = 16; j < 80; j++) {
            const s0 = Uint64.xor(Uint64.xor(Uint64.rotr(w[j - 15], 1), Uint64.rotr(w[j - 15], 8)), Uint64.shr(w[j - 15], 7));
            const s1 = Uint64.xor(Uint64.xor(Uint64.rotr(w[j - 2], 19), Uint64.rotr(w[j - 2], 61)), Uint64.shr(w[j - 2], 6));
        ...
        // 主压缩函数
        for (let j = 0; j < 80; j++) {
            const S1 = Uint64.xor(Uint64.xor(Uint64.rotr(e, 14), Uint64.rotr(e, 18)), Uint64.rotr(e, 41));
            const ch = Uint64.xor(Uint64.and(e, f), Uint64.and(Uint64.xor(e, new Uint64(0xFFFFFFFF, 0xFFFFFFFF)), g));
            const temp1 = Uint64.add(Uint64.add(Uint64.add(Uint64.add(h_val, S1), ch), K[j]), w[j]);
            const S0 = Uint64.xor(Uint64.xor(Uint64.rotr(a, 28), Uint64.rotr(a, 34)), Uint64.rotr(a, 39));
            const maj = Uint64.xor(Uint64.xor(Uint64.and(a, b), Uint64.and(a, c)), Uint64.and(b, c));
