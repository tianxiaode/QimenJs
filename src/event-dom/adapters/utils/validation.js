...
exports.validateDoubleTap = validateDoubleTap;
 ...
 */
function validateDoubleTap(now, lastTapTime, currentX, currentY, lastTapX, lastTapY, maxInterval, maxDistance) {
    if (now - lastTapTime >= maxInterval) {
        return false;
    }
    return utils_1.geometry.isWithinRadius({ x: currentX, y: currentY }, { x: lastTapX, y: lastTapY }, maxDistance);
}
/**
 * 验证长按（long press）手势
 ...
 */
function validateLongPress(startX, startY, currentX, currentY, maxDistance) {
    return utils_1.geometry.isWithinRadius({ x: currentX, y: currentY }, { x: startX, y: startY }, maxDistance);
}
/**
 * 验证滑动（swipe）手势
 ...
 */
function validateSwipe(distance, duration, minDistance, maxDuration, minVelocity) {
    if (duration >= maxDuration || distance < minDistance) {
        return false;
    }
    const velocity = utils_1.geometry.calculateVelocity(distance, duration);
 ...
 */
function validateTap(duration, distance, maxDuration, maxDistance) {
