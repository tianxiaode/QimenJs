...
const GestureError_1 = require("@/error/GestureError");
...
class GestureProcessor {
    ...
    start(input) {
        ...
        if (input.x == null || !Number.isFinite(input.x)) {
            throw new GestureError_1.GestureError('x must be a finite number', codes_1.KernelErrorCode.GESTURE_RECOGNITION_ERROR, {
        ...
        if (input.y == null || !Number.isFinite(input.y)) {
            throw new GestureError_1.GestureError('y must be a finite number', codes_1.KernelErrorCode.GESTURE_RECOGNITION_ERROR, {
                processor: this.constructor.name,
                semantic: this.semantic,
                inputValue: input.y,
            });
