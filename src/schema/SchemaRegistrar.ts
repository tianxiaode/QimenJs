...
export class SchemaRegistrar extends RegistrarBase<SchemaStorage> {
    ...
    register(arg1: RegistrSchema | string, arg2?: FieldDefinition[]): void {
        ...
        if (typeof arg1 === 'string') {
            ...
            if (this.storage.schemas.has(schema.name)) {
                console.warn(`[SchemaRegistrar] Schema "${schema.name}" is already registered, overwriting`);
            }
    ...
    private compileSchema(key: string, schema: RegistrSchema): SchemaCache {
        ...
        if (schema.extends) {
            ...
            if (baseSchema) {
                this.processFieldBatch(baseSchema.fields || [], fieldMap, searchFields, resolvedRules, overrides);
            }
        }
        ...
        if (schema.mixins) {
            schema.mixins.forEach(mKey => {
                ...
                if (mixinFields) {
                    this.processFieldBatch(mixinFields, fieldMap, searchFields, resolvedRules, overrides);
                }
            });
        }
        
        // 4. 处理本地字段
        this.processFieldBatch(schema.fields || [], fieldMap, searchFields, resolvedRules, overrides);
