...
export class EventEngine {
        ...
        for (const [nodeName, decl] of Object.entries(tplEvents)) {
            if (Array.isArray(decl)) {
                ...
                for (const [event, action] of Object.entries(eventDecls)) {
                    const rule = EventEngine._compileEventAction(nodeName, event, action);
                    if (rule) rules.push(rule);
                }

                if ($items) {
                    EventEngine._compileItemEvents(rules, nodeName, $items);
                }
            }
        }
    ...
    private static _compileEventAction(
        ...
        const handler = EventEngine._resolveHandlerName(nodeName, event, action.handler);
            ...
            for (const [event, action] of Object.entries(eventDecls)) {
                ...
                if (rule) {
                    ...
                    if (defaultEventData?.length) {
                        rule.data = EventEngine._mergeData(defaultEventData, rule.data);
                    }
    ...
    static bindDelegatedEvents(instance: any): void {
        ...
        for (const eventType of allEventTypes) {
            ...
            instance.on(domEventKey, (domEvt: any) => {
                EventEngine.handleDelegatedEvent(instance, domEvt, rules);
            });
        }
        ...
        for (const rule of rules) {
            ...
            const callback = (domEvt: any) => {
                EventEngine._dispatchRule(instance, rule, domEvt);
            };
    ...
    static handleDelegatedEvent(instance: any, domEvt: any, rules: DelegatedEventRule[]): void {
        ...
        for (const rule of rules) {
            ...
            if (rule.itemType) {
                ...
                EventEngine._dispatchRule(instance, rule, domEvt, itemInfo);
    ...
    static _dispatchRule(
        ...
        const buildPayload = (eventType: EventDataType): any => {
            const fields = EventEngine._resolveDataFields(rule.data, eventType);
            const extraData = fields
                ? EventEngine._collectDataFields(instance, itemPayload, fields)
                : {};
            const eventData = EventEngine._collectEventData(
                instance,
                rule.nodeName,
                rule.event,
                eventType
        ...
        if (rule.handler) {
            ...
            if (keyValue) {
                handlerName = EventEngine._resolveHandlerName(keyValue, rule.event, true)!;
            }
        ...
        if (rule.emits?.length) {
            ...
            for (const emitName of rule.emits) {
                if (keyValue) {
                    ...
                    const specificCtx = EventEngine._buildForwardContext(
                        instance,
                        specificName,
                        payload,
                        instance.eventKey ?? '',
                ...
                const ctx = EventEngine._buildForwardContext(
                    instance,
                    emitName,
                    payload,
                    instance.eventKey ?? '',
...
// 向后兼容别名
export const DelegatedEventEngine = EventEngine;
