 ...
 * 解析 i18n 值
 *
 * 如果值以 '@' 开头（且不是 '@@'），去掉前缀后调用 i18n.t() 翻译并返回；
 * 以 '@@' 开头时去掉一个 '@' 原样返回（转义）；
 * 否则原样返回。
 *
 * @param value - 原始值，可能带 '@' 前缀
 * @returns 翻译后的文本或原始值
 */
export function resolveI18nValue(value) {
    if (value.startsWith('@@')) {
 ...
 * 直接翻译 i18n key
 *
 * 不需要 '@' 前缀，直接调用 i18n.t(key) 翻译。
 * 若 key 带 '@' 前缀会自动去掉。
 * 适用于已知是 key 的场景（如错误代码、枚举值），
 * 不必手动拼接前缀。
 *
 * @param key - i18n key，如 'error.network.timeout' 或 '@error.network.timeout'
