 ...
 *
 * 使用方式：
 * 1. 模板中定义 { tag: 'div', name: 'xxx:expand', className: 'q-expand-arrow' }
...
export interface ExpandArrowConfig {
    ...
    arrowEvent?: string;
    /** 箭头节点名称，默认 'expand'（对应模板中 name 的 name 部分） */
    arrowName?: string;
}
...
export const ExpandArrowAbility: AbilityDefinition = {
    ...
    initExpandArrow(config?: ExpandArrowConfig): void {
        ...
        (this as any)._expandArrowEl = arrowEl;

        // 设置初始状态类
        arrowEl.classList.add('q-expand-arrow', `q-expand-arrow--${state}`);

        // 绑定点击
        arrowEl.addEventListener('click', (e: Event) => {
            e.stopPropagation();
    ...
    setExpandArrowState(state: 'collapsed' | 'expanded'): void {
        ...
        // 更新 CSS 状态类（使用缓存的 el）
        const el = (this as any)._expandArrowEl as HTMLElement | undefined;
        if (el) {
