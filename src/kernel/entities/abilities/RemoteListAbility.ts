...
export class RemoteListAbility extends AbilityBase<IEntityManagerBase> {
    // 1. 定义防抖函数，包装真正的执行逻辑
    ...
    protected onAttach(): void {
        ...
        host.list = (forceRefresh: boolean = false): Promise<any[]> => {
     ...
     * 真正的执行逻辑
     */
    private async actualRun(forceRefresh: boolean): Promise<any[]> {
        const { host } = this;
