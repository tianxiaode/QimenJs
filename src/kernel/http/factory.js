...
const tasks_1 = require("@orbitjs/tasks");
/**
 * HttpFactory 类
 *
...
class HttpFactory {
    ...
    static createRetryTask(method, url, options, domain = 'default') {
        ...
        let client = new HttpClient_1.HttpClient(domain);
        const execute = async () => {
            while (true) {
     ...
     */
    static schedulePolling(method, url, pollingOptions, domain = 'default') {
        // 解构轮询选项
        const { interval = 5000, // 默认轮询间隔 5 秒
        ...
        // 直接注入全局任务队列
        tasks_1.globalTaskQueue.addTask(taskFn, priority, maxRetries, retryDelay, true, // isPolling = true，标识这是一个轮询任务
        interval);
    }
}
