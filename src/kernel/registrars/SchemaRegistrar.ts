...
import { SchemaRegistrarError, KernelErrorCode } from '../errors';
 ...
 * - 提供统一的API来访问不同类型的数据
 */
export class SchemaRegistrar extends RegistrarBase<Map<string, RegistrSchema>> {
    ...
    get<T = RegistrSchema>(name: string, type: 'schema' | 'field' = 'schema'): T {
        if (type === 'field') {
            ...
            if (!fieldDef) {
                throw new SchemaRegistrarError(
                    `Field template with name "${name}" not found.`,
                    KernelErrorCode.SCHEMA_NOT_FOUND,
                    { fieldName: name, type: 'field' }
                );
            }
        ...
        if (!entry) {
            throw new SchemaRegistrarError(
                `Schema with name "${name}" not found.`,
                KernelErrorCode.SCHEMA_NOT_FOUND,
                { schemaName: name, type: 'schema' }
            );
        }
        return entry as any;
    }
