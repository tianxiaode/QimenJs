...
exports.HashTaskResources = void 0;
const errors_1 = require("../errors");
const ResourceNotAcquiredError_1 = require("../errors/ResourceNotAcquiredError");
/**
 * 哈希任务资源管理类
 *
...
class HashTaskResources {
     ...
     */
    constructor(memoryManager, workerPool) {
        this.memoryManager = memoryManager;
        this.workerPool = workerPool;
        this.acquired = false; // 标记资源是否已成功获取
    }
    /**
     * 核心：申请计算所需的资源（内存 + 线程）
     *
     ...
     */
    async acquire(scriptSource, memoryBytes) {
        if (this.acquired)
            return; // 防止重复获取资源
        try {
            // 按顺序获取内存和Worker资源
     ...
     */
    async acquireMemory(memoryBytes) {
        try {
            this.memoryTicket = await this.memoryManager.acquire(memoryBytes);
        }
        catch (err) {
            throw new errors_1.ResourceUnavailableError('memory', {
     ...
     */
    async acquireWorker(scriptSource) {
        try {
            this.worker = await this.workerPool.acquire(scriptSource);
        }
        catch (err) {
            throw new errors_1.ResourceUnavailableError('worker', {
                originalError: err instanceof Error ? err.message : err,
            });
        }
    }
    ...
    getWorker() {
        if (!this.worker) {
            throw new ResourceNotAcquiredError_1.ResourceNotAcquiredError();
        }
        return this.worker;
    }
    /**
    ...
    snapshot() {
        ...
        return {
            // ✅ 修正：如果有内存票据，返回具体的字节数
            memoryBytes: (_a = this.memoryTicket) === null || _a === void 0 ? void 0 : _a.bytes,
            hasWorker: !!this.worker, // 确保返回布尔值
        };
    }
}
exports.HashTaskResources = HashTaskResources;
//# sourceMappingURL=HashTaskResources.js.map