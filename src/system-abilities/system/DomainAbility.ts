...
export class DomainAbility extends AbilityBase {
    ...
    protected expose(proxy: AbilityProxy): IExposeResult {
        return {
             ...
             * 域配置属性
             * 
             * 使用 getter 延迟获取配置，并在 getter 中访问 proxy.host
             */
            domainConfig: {
                get: (): DomainConfig => {
                    // 1. 尝试从缓存获取
                    let config = proxy.host.getStatic(DOMAIN_CACHE_SYMBOL);
                    
                    // 2. 如果没有缓存，则初始化
                    if (!config) {
                        const domainName = proxy.host.domain;
                        if (domainName) {
                            config = DomainRegistrar.getInstance().get(domainName);
                            proxy.host.setStatic(DOMAIN_CACHE_SYMBOL, config);
                            proxy.host.logger?.debug?.(`Domain [${domainName}] initialized and cached.`);
                        }
                    }
