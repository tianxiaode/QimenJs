...
exports.TreeLifecycleAbility = void 0;
const composable_1 = require("../../../composable");
class TreeLifecycleAbility extends composable_1.AbilityBase {
    expose() {
        const { host } = this;
        return {
            removeNode: (id) => this.removeNode(id),
            moveNode: (id, targetPid) => this.moveNode(id, targetPid),
            syncChildren: (pid, newData) => this.syncChildren(pid, newData),
            getChildren: (pid, predicate) => this.getChildren(pid, predicate),
        };
    }
    removeNode(id) {
        const host = this.host;
        const targetNode = host.nodes.get(id);
        if (!targetNode)
            return;
        ...
        if (siblings) {
            host.hierarchy.set(pid, siblings.filter(childId => childId !== id));
        }
        // 2. 批量删除自己和所有子孙 (线性扫描)
        // 凡是路径为 "1.2" 或以 "1.2." 开头的节点全部删除
        host.nodes.forEach((node, nodeId) => {
    ...
    moveNode(id, targetPid) {
        ...
        const targetParent = host.nodes.get(targetPid);
        const newParentPath = (targetParent === null || targetParent === void 0 ? void 0 : targetParent._path) || '';
        ...
        if (oldSiblings) {
            host.hierarchy.set(oldPid, oldSiblings.filter((sid) => sid !== id));
        }
        // 加入新父级
    ...
    getChildren(pid = this.host.root, predicate) {
        const host = this.host;
        // 从索引中获取 ID 列表
...
exports.TreeLifecycleAbility = TreeLifecycleAbility;
//# sourceMappingURL=TreeLifecycleAbility.js.map