...
exports.CommonParamsEnricherHandler = void 0;
const CommonParamsEnricherHandler = async (context) => {
    const { commonParams, commonBody } = context.config;
    // 1. 合并公共 Query 参数 (如 appId)
    if (commonParams) {
        // 修正：执行函数获取结果，如果没有结果则取原对象
        const common = typeof commonParams === 'function' ? commonParams() : commonParams;
        context.http.queryParams = { ...common, ...context.http.queryParams };
    }
    // 2. 合并公共 Body 参数
    if (commonBody) {
        // 修正：执行函数获取结果
...
exports.CommonParamsEnricherHandler = CommonParamsEnricherHandler;
//# sourceMappingURL=CommonParamsEnricher.js.map