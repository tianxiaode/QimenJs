...
exports.XhrTransportHandler = void 0;
const XhrTransportHandler = async (context) => {
    // 1. 自治判定：只有上传任务才由 XHR 处理
    if (!context.metadata.isUpload && !context.metadata.isDownload)
        return;
    // 2. 初始化中断上下文
    ...
    return new Promise(resolve => {
        ...
        xhr.onabort = () => handleError(internalController.signal.reason === 'timeout' ? 'timeout' : 'cancelled');
        // 8. 资源清理与释放
        const finalize = () => {
...
exports.XhrTransportHandler = XhrTransportHandler;
/**
 * 解析响应头
 *
 * 将原生响应头字符串转换为普通对象格式
