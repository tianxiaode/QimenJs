...
exports.FlatLocalMutationAbility = void 0;
const composable_1 = require("../../../composable");
class FlatLocalMutationAbility extends composable_1.DebounceAbilityBase {
    expose() {
        const { host } = this;
        const { state } = host;
        ...
        return {
             ...
             * 保存：将本地所有变更同步到远程
             */
            save: (isBatch = false) => debouncedSave(isBatch),
        };
    }
    async internalSave(isBatch = false) {
        const { host } = this;
        const { state } = host;
        if (!state.hasChanges)
            return;
...
exports.FlatLocalMutationAbility = FlatLocalMutationAbility;
//# sourceMappingURL=FlatLocalMutationAbility.js.map