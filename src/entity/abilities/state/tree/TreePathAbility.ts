    ...
    protected expose(proxy: AbilityProxy): IExposeResult {
        return {
            ingest: (data: IEntity | IEntity[], manualParentId?: string | number | null) =>
                proxy.self.ingest(data, manualParentId),
            rebuildDescendantsPaths: (pid: any, parentPath: string, nextDepth: number) =>
                proxy.self.rebuildDescendantsPaths(pid, parentPath, nextDepth),
            toggleExpand: (id: string | number | IEntity, expanded?: boolean) =>
                proxy.self.toggleExpand(id, expanded),
            toggleLeaf: (id: string | number | IEntity, leaf?: boolean) => proxy.self.toggleLeaf(id, leaf),
        };
    }
