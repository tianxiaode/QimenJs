...
const RuleAlignmentProcessor = async (context) => {
    ...
    if (rule.type === 'password') {
        ...
        const systemRegistrar = registry_1.SystemRegistrar.getInstance();
        const { minLength, maxLength, uppercase, lowercase, digit, specialChar } = systemRegistrar.get('password');
