...
import { validateHasKeys } from '../../../../../../../src/utils/validation/validators/extensions/object/keys';

describe('validateHasKeys函数测试', () => {
  it('当对象包含所有必需键时验证通过', () => {
    const result = validateHasKeys({ name: 'John', age: 30 }, { keys: ['name', 'age'], allErrors: false }, {});
  ...
  it('当对象缺少必需键时验证失败', () => {
    const result = validateHasKeys({ name: 'John' }, { keys: ['name', 'age'], allErrors: false }, {});
  ...
  it('当规则中keys为无效类型时验证失败', () => {
    const result = validateHasKeys({ name: 'John' }, { keys: 123 as any, allErrors: false }, {});
  ...
  it('当规则中keys为undefined时验证失败', () => {
    const result = validateHasKeys({ name: 'John' }, { keys: undefined as any, allErrors: false }, {});
  ...
  it('当对象包含所有必需键（包括字符串数字键）时验证通过', () => {
    const result = validateHasKeys({ '1': 'first', 'name': 'John' }, { keys: ['1', 'name'], allErrors: false }, {});

    expect(result).toBeNull();
  });

  it('当对象缺少数字键时验证失败', () => {
    const result = validateHasKeys({ '2': 'second', 'name': 'John' }, { keys: ['1', 'name'], allErrors: false }, {});
  ...
  it('当对象有原型属性但没有自身属性时验证失败', () => {
    ...
    const result = validateHasKeys(obj, { keys: ['inherited', 'ownProp'], allErrors: false }, {});
  ...
  it('当allErrors为true且缺少多个键时返回所有错误', () => {
    const result = validateHasKeys({ name: 'John' }, { keys: ['name', 'age', 'email'], allErrors: true }, {});
  ...
  it('当allErrors为false且缺少多个键时只返回第一个错误', () => {
    const result = validateHasKeys({ name: 'John' }, { keys: ['name', 'age', 'email'], allErrors: false }, {});
  ...
  it('应该正确传递上下文信息', () => {
    ...
    const result = validateHasKeys({ name: 'John' }, { keys: 'age', allErrors: false }, context);
