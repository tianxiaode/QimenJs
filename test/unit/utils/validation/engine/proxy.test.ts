import { validator, assert, createValidatorProxy, createAssertProxy } from '@/utils/validation/engine/proxy';
import { Validator, ValidationErrorContext } from '@/utils/validation/core';

// Mock Validator.executeValidator 方法，因为我们需要测试代理行为
jest.mock('@/utils/validation/core', () => {
    const originalModule = jest.requireActual('@/utils/validation/core');
...
describe('proxy.ts - 验证器代理测试', () => {
    ...
    describe('assert 代理', () => {
        ...
        it('当验证失败时应该抛出异常', () => {
            ...
            jest.spyOn(require('@/utils/validation/core').ValidationErrorBuilder, 'throwIfAny')
                .mockImplementation(() => {
                    throw mockError;
                });
