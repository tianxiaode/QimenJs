...
describe('setJson', () => {
  ...
  it('should call set with JSON stringified value', () => {
    ...
    expect(set).toHaveBeenCalledWith('test', JSON.stringify(testObj), undefined, '/', undefined, false, 'Lax');
  });
  ...
  it('should handle array values', () => {
    ...
    expect(result).toBe(true);
    expect(set).toHaveBeenCalledWith('array', JSON.stringify(testArray), undefined, '/', undefined, false, 'Lax');
  });
  ...
  it('should pass all parameters to set function', () => {
    ...
    const result = setJson('test', { data: 'value' }, futureDate, '/path', 'example.com', true, 'Strict');
    
    expect(result).toBe(true);
    expect(set).toHaveBeenCalledWith('test', JSON.stringify({ data: 'value' }), futureDate, '/path', 'example.com', true, 'Strict');
  });
});