...
import { ItemGroupComponent } from '@/component/itemgroup/ItemGroupComponent';
...
describe('ItemGroupComponent', () => {
    ...
    describe('constructor', () => {
        it('创建 el 并添加 q-itemgroup 类', () => {
            const group = new ItemGroupComponent({ itemType: 'MockItem' }) as any;
        ...
        it('设置纵向排列', () => {
            const group = new ItemGroupComponent({ itemType: 'MockItem', direction: 'vertical' }) as any;
            expect(group.direction).toBe('vertical');
            expect(group.el.classList.contains('q-itemgroup--vertical')).toBe(true);
        });

        it('设置 gap', () => {
            const group = new ItemGroupComponent({ itemType: 'MockItem', gap: '8px' }) as any;
            expect(group.gap).toBe('8px');
        });

        it('设置 eventKey', () => {
            const group = new ItemGroupComponent({ itemType: 'MockItem', eventKey: 'tabs' }) as any;
            expect(group.eventKey).toBe('tabs');
        });

        it('设置额外 CSS 类名', () => {
            const group = new ItemGroupComponent({ itemType: 'MockItem', cls: 'q-itemgroup--divider custom-class' }) as any;
        ...
        it('通过 items 初始化子项', () => {
            const group = new ItemGroupComponent({
                itemType: 'MockItem',
    ...
    describe('add', () => {
        ...
        it('itemType 未设置时返回 null', () => {
            const group = new ItemGroupComponent() as any;
            const item = group.add({ label: 'A' });
