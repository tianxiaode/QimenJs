...
describe('DefaultWorkerPool', () => {
    ...
    test('should create new workers up to max limit', async () => {
        ...
        const workers = await Promise.all([pool.acquire(scriptSource), pool.acquire(scriptSource)]);
    ...
    test('should queue when max workers reached', async () => {
        ...
        // Acquire max workers
        await Promise.all([pool.acquire(scriptSource), pool.acquire(scriptSource)]);
    ...
    test('should throw error when acquiring from destroyed pool', async () => {
        ...
        await expect(pool.acquire('console.log("test");')).rejects.toThrow(
            'WorkerPool is destroyed'
        );
    });
});
