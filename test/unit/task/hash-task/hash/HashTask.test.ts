import { HashTask } from '@/task/hash-task/hash/HashTask';
...
import { WorkerPool } from '@/task/hash-task/worker';

// Mock dependencies
jest.useFakeTimers();
// 避免导入会触发HashWorker执行的模块
jest.mock('@/task/hash-task/hash/HashTaskRunner');
jest.mock('@/task/hash-task/hash/HashTaskState');
jest.mock('@/task/hash-task/hash/HashTaskProgress');
