...
jest.mock('@/component-core/engine/TemplateCompiler', () => ({
    ...
    compileTemplate: jest.fn((tpl: any, logger: any) => {
        const nodeMetas: any = {
            root: { name: 'root', tag: tpl?.tag || 'div', componentClass: tpl?.type },
        };
        const indexPath: any = { root: [] };
        if (tpl?.children) {
            for (const child of tpl.children) {
                if (child.name) {
                    nodeMetas[child.name] = {
                        name: child.name,
                        tag: child.tag,
                        componentClass: child.type,
                    };
...
describe('template-factory', () => {
    ...
    describe('createDerivedInnerClass (optimized)', () => {
        it('reuses parent _tpl and _body', () => {
            ...
            } = require('@/component-core/engine/TemplateFactory');
            const Parent = require('@/composable').ComposableBase;
        ...
        it('merges body (child overrides parent)', () => {
            ...
            const Parent = require('@/composable').ComposableBase;
            const ParentClass = createInnerClass(
                Parent,
                { tag: 'div' },
                {
                    type: 'Parent',
    ...
    describe('lifecycle hooks (default overrides)', () => {
        ...
        it('multi-level lifecycle hooks auto chain', () => {
            ...
            const Parent = require('@/composable').ComposableBase;
            const BaseClass = createInnerClass(
                Parent,
                { tag: 'div' },
                {
                    type: 'Base',
    ...
    describe('replaces: direct override without chaining', () => {
        it('replaces excludes method from override queue', () => {
            const {
                createInnerClass,
                createDerivedInnerClass,
            } = require('@/component-core/engine/TemplateFactory');
        ...
        it('replaces method directly overrides on prototype', () => {
            const {
                createInnerClass,
                createDerivedInnerClass,
            } = require('@/component-core/engine/TemplateFactory');
            ...
            expect(typeof DerivedClass.prototype.onAfterInit).toBe('function');
        });

        it('replaces only affects specified methods, others still chain', () => {
            const {
                createInnerClass,
                createDerivedInnerClass,
            } = require('@/component-core/engine/TemplateFactory');
        ...
        it('replaces with custom overrides list', () => {
            const {
                createInnerClass,
                createDerivedInnerClass,
            } = require('@/component-core/engine/TemplateFactory');
            ...
            const ParentClass = createInnerClass(
                ...
                {
                    type: 'Parent',
                    overrides: ['onInitState', 'onAfterInit', 'customHook'],
                    onInitState() {
                        return { parent: true };
                    },
                    onAfterInit() {},
            ...
            expect(queues.onInitState.length).toBe(2);
            expect(queues.onAfterInit.length).toBe(2);
            expect(queues.customHook).toBeUndefined();
        });
    });
    ...
    describe('engine pure function verification', () => {
        ...
        it('multi-level replace chain: BaseInput -> NumberInput -> CurrencyInput', () => {
            ...
            const BaseInput = createInnerClass(
                Parent,
                { tag: 'div' },
                {
                    type: 'Input',
