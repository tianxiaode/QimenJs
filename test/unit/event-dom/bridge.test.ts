import { EventBus, EventScope } from '../../../src/event-core';
...
describe('bridgeDomEvent', () => {
  ...
  it('应该在目标元素上添加事件监听器', () => {
    const { bridgeDomEvent } = require('../../../src/event-dom/bridge');
    bridgeDomEvent(scope, bus, element, 'click', 'custom:click');

    expect(element.eventListeners['click']).toBeDefined();
    expect(element.eventListeners['click'].length).toBe(1);
  });

  it('应该在 DOM 事件触发时发出 EventBus 事件', () => {
    ...
    expect(emitSpy).toHaveBeenCalledWith('custom:click', mockEvent);
  });
  ...
  it('应该支持事件监听器选项', () => {
    ...
    const { bridgeDomEvent } = require('../../../src/event-dom/bridge');
    bridgeDomEvent(scope, bus, element, 'click', 'custom:click', options);
