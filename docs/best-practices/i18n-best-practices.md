...
## 4. 远程资源用 @orbitjs/http 加载，不要在 i18n 中内置网络请求

```typescript
// 正确 - 用 @orbitjs/http 加载远程资源
...
**原因**：i18n 的职责是存消息和查消息，不是发网络请求。项目已有 `@orbitjs/http` 处理所有 HTTP 通信（含拦截器、缓存、重试等），在 i18n 中再实现一套 fetch 是重复且不一致的。`inject()` 是唯一的消息入口，任何来源的消息都通过它注入。
