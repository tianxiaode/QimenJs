...
因此采用**方案 A**：TokenRefreshHandler 由 `@qimenjs/oauth2` 自行注册到 HttpActionRegistrar，引入包即生效，不引入则零影响。

这与 `@qimenjs/pattern` 引入即自动注册验证模式、`@qimenjs/data-processor-abp` 引入即自动注册 ABP 处理器的模式完全一致。
...
引入 `@qimenjs/oauth2` 后自动生效，无需额外配置：
...
```
@qimenjs/oauth2 (L3)
  ├── @qimenjs/http        HTTP 管道扩展（TokenRefreshHandler）
  ├── @qimenjs/registry    DomainRegistrar.updateToken()
  ├── @qimenjs/events      事件通知（token 过期/刷新）
  └── @qimenjs/cache       Token 存储（TTL 过期检查）
```
...
| 层级 | 职责 | 包 |
|------|------|-----|
| 底层 | Token 存储 + 注入 | `@qimenjs/registry` (DomainConfig.token) + `@qimenjs/http` (TokenInjector) |
| 上层 | Token 获取 + 刷新 + 401 重试 | `@qimenjs/oauth2` (OAuth2Manager + TokenRefreshHandler) |
